...
public class AnytownLibrary implements Library
{
     ...
     * @param  dateCheckedOut the date item was checked out. 
     ...
     * @param callNumber the call number
     * @param copy the copy number
     * @param borrower the borrower idof
    ...
    int copy, String borrower, GregorianCalendar currentDate)
    {
        ...
        if (item.getBorrower() == borrower && item instanceof Book
                && 0 <= item.getDueDate().compareTo(currentDate))        
        {
            item = items[index];
            newDate = item.getDueDate();
    ...
    public Book[] renew(String borrower, GregorianCalendar currentDate)
    {
        ...
        for (int i = 0; i < numItems; i++)
        {
            if (borrower.equals(items[i].getBorrower()) 
                       && items[i] instanceof Book
                       && 0 <= items[i].getDueDate().compareTo(currentDate))
            {
                matchCount++;
            } // throw borrower not found, and no books borrowed
        }
        //creates array
        Book[] renewed = new Book[matchCount];
        matchCount = 0;
        //adds item to array
        for (int i = 0; i < numItems; i++)
        {
            if (borrower.equals(items[i].getBorrower()) 
                        ...
                        && 0 <= items[i].getDueDate().compareTo(currentDate))
            {
                String call = items[i].getCallNumber();
                int copy = items[i].getCopyNumber();
                renew(call, copy, borrower, currentDate);

                renewed[matchCount++] = (Book)items[i];
            }
        }
        return renewed;
